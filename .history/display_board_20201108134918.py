...
class Display_board():
    ...
    def draw(self, grid, cell):
        ...
        indent = []
        for i in self.numbers(10):
            indent-append(i)
        for indent in self.numbers(7):
            print("\nline: ---", i, "---")
            if i % 3 == 0:
                print("thick")
                thick = THICK_LINE
            ...
            pg.draw.line(self.screen, BLACK, (TOP_LX, 
                                            TOP_LY + i * BLOCK_SIZE + indent), 
                                            (TOP_RX, 
                                            TOP_RY + i * BLOCK_SIZE + indent), thick)
            pg.draw.line(self.screen, BLACK, (TOP_LX + i * BLOCK_SIZE + indent, 
                                            TOP_LY), 
                                            (BOT_LX + i * BLOCK_SIZE + indent, 
                                            BOT_LY), thick)
