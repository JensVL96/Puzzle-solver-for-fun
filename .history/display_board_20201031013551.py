...
class Display_board():
    ...
    def glow(self, x, y):
        if x == 0 and y == 0:
            return
        print("glowing yet", x, x * BLOCK_SIZE, TOP_LX + x * BLOCK_SIZE)
        start_pos_x = TOP_LX + x * BLOCK_SIZE
        start_pos_y = TOP_LY + y * BLOCK_SIZE
        end_pos_x = x + BLOCK_SIZE
        end_pos_y = y + BLOCK_SIZE
