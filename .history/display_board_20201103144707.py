                    ...
                    if type(grid[i][j]) != int:
                        ...
                        for number in grid[i][j]:
                            if iteration % 3 == 0:      # checking if last in line: 3, 6
                                ...
                                self.screen.blit(text1, ((TOP_LX + i * BLOCK_SIZE) + indent_h, (TOP_LY + j * BLOCK_SIZE) + new_line * indent_v))
                            else:
                                text1 = self.font_cdt.render(str(number), 1, BLACK)
                                self.screen.blit(text1, ((TOP_LX + i * BLOCK_SIZE) + iteration * indent_h, (TOP_LY + j * BLOCK_SIZE) + new_line * indent_v + 10))
