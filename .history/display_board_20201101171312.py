...
class Display_board():
    def __init__(self, screen):
        ...
        self.font_cdt = pg.font.SysFont("comicsans", NUMBER_SIZE) 
            ...
            for j in range (9):
                if grid[i][j] != 0:
                    if type(grid[i][j]) != int:
                        text1 = self.font_cdt.render(str(grid[i][j]), 1, BLACK)
                        self.screen.blit(text1, (TOP_LX + i * BLOCK_SIZE + 1, TOP_LY + j * BLOCK_SIZE + 1))
                    else:
