...
class Display_board():
            ...
            for j in range (9):
                if grid[i][j] != 0:
                    if type(grid[i][j]) != int:
                        candidates = self.sort_candidates(grid[i][j])
                        self.draw_candidates(candidates, cell)
                    else:
                        text1 = self.font_num.render(str(grid[i][j]), 1, BLACK)
                        self.screen.blit(text1, (TOP_LX + i * BLOCK_SIZE + indent[i] * BOX_INDENT + 15, TOP_LY + j * BLOCK_SIZE + indent[j] * BOX_INDENT + 14))
    ...
    def draw_candidates(self, grid, cell):
        ...
        for number in grid:
            ...
            # else:
            text1 = self.font_cdt.render(str(number), 1, BLACK)
            self.screen.blit(text1, (cell[0] + ((iteration - 1) * indent) + 3.5, cell[1] + ((new_line - 1) * indent) + 2))
