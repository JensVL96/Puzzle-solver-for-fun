                    ...
                    if type(grid[i][j]) != int:
                        ...
                        for number in grid[i][j]:
                            if iteration % 3 == 1:    # Checking if first in line: 1, 4, 7
                                ...
                                self.screen.blit(text1, (cell[0] + 3, cell[1] + ((new_line - 1) * indent_v) + 2))
                            else:
                                print(new_line - 1, new_line * indent_v)
                                text1 = self.font_cdt.render(str(number), 1, BLACK)
                                self.screen.blit(text1, (cell[0] + ((iteration - 1) * indent_h) + 3, cell[1] + ((new_line - 1) * indent_v) + 2))
