...
class Display_board():
    ...
    def draw_val(self, val, x, y, thick):
        font_val = pg.font.SysFont("comicsans", thick)
        text1 = font_val.render(str(val), 1, BLACK)
        self.screen.blit(text1, (x * BLOCK_SIZE + 15, y * BLOCK_SIZE + 15))

    def on_mouse_press(self, x, y, symbol, modifier):
        pass

    def draw(self, grid, thick):
        font_val = pg.font.SysFont("comicsans", thick)
        for i in range (9):
            for j in range (9):
                if grid[i][j] != 0:

                    text1 = font_val.render(str(grid[i][j]), 1, BLACK)
