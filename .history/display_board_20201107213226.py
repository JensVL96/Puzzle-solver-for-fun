...
class Display_board():
    ...
    def draw(self, grid, cell):
        ...
        for i in range(10):
            ...
            # else:
            #     size = 0
            check_diffX = TOPx_coord
            TOPx_coord += BLOCK_SIZE + size

            check_diffY = TOPy_coord
            TOPy_coord += BLOCK_SIZE + size

            #       thick           Thick     thin    thin  thick   thin    thin  thick   thin    thin  thick
            # TOP_LX -> TOP_RX & ( TOP_LY -> BS + 7 -> BS -> BS -> BS + 7 -> BS -> BS -> BS + 7 -> BS -> BS)
            print("Start horizontal: ", check_diffX, "end: ", TOPx_coord)
            print("line size: ", size, "block size: ", BLOCK_SIZE, "total distance: ", BLOCK_SIZE + size)
            print("Start vertical: ", check_diffY, "end: ", TOPy_coord)
